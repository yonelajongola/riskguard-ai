...
export function RisksPage() {
  ...
  return <div className="page-stack"><PageHeader eyebrow="Enterprise register" title="Risk register" description="Prioritized exposure with ownership, treatment status, and estimated financial impact." actions={<><button type="button" className="button button-secondary" disabled={exporting} onClick={exportExcel}><Download size={16}/> {exporting?"Exporting...":"Export Excel"}</button><Link className="button button-primary" to="/app/assessments/new"><Plus size={16}/> Add risk assessment</Link></>}/>
...
export function RecommendationsPage() {
  ...
  const canManage=user?.roles.some((role)=>["Admin","Risk Manager","Compliance Officer","Security Analyst"].includes(role));
...
export function IncidentsPage() {
  ...
  const canManage=user?.roles.some((role)=>["Admin","Risk Manager","Security Analyst","Compliance Officer","Department Manager"].includes(role));
...
export function ContinuityPage() {
  ...
  return <div className="page-stack"><PageHeader eyebrow="Operational resilience" title="Business continuity" description="Recovery objectives, backup assurance, disaster recovery testing, and downtime exposure." actions={canManage?<button type="button" className="button button-primary" disabled={systems.length===0} onClick={()=>{setSelectedSystem(systems[0]?.id??"");setMode("test")}}><RefreshCw size={16}/> Record recovery test</button>:undefined}/>
    ...
    <Card className="table-card"><div className="card-head"><div><span className="card-kicker">Recovery inventory</span><h2>Critical systems</h2></div>{canManage?<button type="button" className="button button-secondary" onClick={()=>setMode("system")}><Plus size={15}/> Add system</button>:null}</div><div className="table-wrap"><table><thead><tr><th>Critical system</th><th>Owner</th><th>RTO</th><th>RPO</th><th>Backup frequency</th><th>Last DR test</th><th>Readiness</th><th>Status</th></tr></thead><tbody>{systems.map(system=><tr key={system.id}><td><button type="button" className="cell-link bare-button" onClick={()=>{setSelectedSystem(system.id);setTestForm({...testForm,continuityScore:system.continuityScore,status:system.status});setMode("test")}}><span className="type-icon"><ServerCog size={17}/></span><span><strong>{system.name}</strong><small>Record or review a recovery test</small></span></button></td><td>{system.systemOwner}</td><td>{system.recoveryTimeObjectiveHours}h</td><td>{system.recoveryPointObjectiveHours}h</td><td>{system.backupFrequency}</td><td>{formatDate(system.lastDisasterRecoveryTestDateUtc)}</td><td><div className="readiness-cell"><ProgressBar value={system.continuityScore} tone="blue"/><strong>{system.continuityScore}%</strong></div></td><td><StatusBadge status={system.status}/></td></tr>)}</tbody></table></div></Card>
